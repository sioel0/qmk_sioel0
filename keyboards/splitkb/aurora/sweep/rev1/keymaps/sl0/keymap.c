...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[_HYP] = LAYOUT(
      CLOSE , MOVLEFT, MOVDOWN,  MOVUP ,MOVRIGHT, FOCLEFT, FOCDOWN,  FOCUP ,FOCRIGHT, XXXXXXX,
	   FOCWSP1, FOCWSP2, FOCWSP3, FOCWSP4, FOCWSP5, FOCWSP6,OPENTERM, MASTSWP, XXXXXXX, XXXXXXX,
	...
	if(is_keyboard_master()) {
	  // sioel0 logo height 64 bit
		...
		if(default_layer_state == _COL) {
			oled_write_P(PSTR("_COL"), current_layer == _COL);
		} else if(default_layer_state == _QWE) {
			oled_write_P(PSTR("_QWE"), current_layer == _QWE);
		}
		oled_write_P(PSTR("_NAV"), current_layer == _NAV);
		oled_write_P(PSTR("_SYM"), current_layer == _SYM);
		oled_write_P(PSTR("_NUM"), current_layer == _NUM);
		oled_write_P(PSTR("_HYP"), current_layer == _HYP);
		...
		if(oneshot_mods & MOD_MASK_ALT || mods & MOD_MASK_ALT) {
